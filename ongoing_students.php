...
if (isset($_GET['category']) && !empty(trim($_GET['category']))) {
    ...
    $sql = "SELECT student_id, name, email, phone_number, course_status, course_category
            FROM students
            WHERE course_status = 'ongoing' AND course_category LIKE '%$search_category%'
            ORDER BY name ASC";
} else {
    $sql = "SELECT student_id, name, email, phone_number, course_status, course_category
            FROM students
    ...
    <?php if ($result->num_rows > 0): ?>
        <table>
            ...
            <tr>
                ...
                <td><b style="color:orange;"><?= ucfirst($row['course_status']) ?></b></td>
            </tr>
            <?php endwhile; ?>
        </table>
    <?php else: ?>
        <p class="error">⏳ No ongoing students found.</p>
    <?php endif; ?>
</div>
