...
<title>IP Geolocation — AR TECH SOLUTION</title>
...
<main class="main" id="mainContent">
    <div class="locator-card">
        ...
        <div class="form-group">
            ...
            <input type="text" id="ipInput" placeholder="Enter IP address (leave empty for your own IP)">
        </div>
        <button class="search-btn" id="lookupBtn">🔍 Lookup Location</button>
        <div id="result"></div>
    </div>
</main>
...
// IP Geolocation logic using freegeoip.app (HTTPS supported, no API key required)
const lookupBtn = document.getElementById('lookupBtn');
const ipInput = document.getElementById('ipInput');
const resultDiv = document.getElementById('result');

// Helper function to display location data from the new API
function displayLocation(data) {
    // Map the response fields to our display format
    const html = `
        <div class="result-card">
            <h3>📍 Location Details</h3>
            <div class="result-grid">
                <div class="result-item"><span class="result-label">IP Address:</span><span class="result-value">${data.ip || 'N/A'}</span></div>
                <div class="result-item"><span class="result-label">Country:</span><span class="result-value">${data.country_name || data.country || 'N/A'} (${data.country_code || 'N/A'})</span></div>
                <div class="result-item"><span class="result-label">Region:</span><span class="result-value">${data.region_name || data.region || 'N/A'} (${data.region_code || 'N/A'})</span></div>
                <div class="result-item"><span class="result-label">City:</span><span class="result-value">${data.city || 'N/A'}</span></div>
                <div class="result-item"><span class="result-label">ZIP Code:</span><span class="result-value">${data.zip_code || data.postal || 'N/A'}</span></div>
                <div class="result-item"><span class="result-label">Latitude / Longitude:</span><span class="result-value">${data.latitude || data.lat || 'N/A'} / ${data.longitude || data.lon || 'N/A'}</span></div>
                <div class="result-item"><span class="result-label">Timezone:</span><span class="result-value">${data.time_zone || data.timezone || 'N/A'}</span></div>
            </div>
        </div>
    `;
...
window.addEventListener('DOMContentLoaded', () => {
    // Use the new API to get current IP and location
    fetch('https://freegeoip.app/json/')
        ...
        .then(data => {
            if (data && data.ip) {
                ...
                resultDiv.innerHTML = '<div class="error-msg">Failed to detect your IP. Please enter manually.</div>';
            }
        })
        .catch(err => {
...
lookupBtn.addEventListener('click', () => {
    ...
    if (ip === '') {
        resultDiv.innerHTML = '<div class="error-msg">Please enter an IP address.</div>';
        return;
    }
    resultDiv.innerHTML = '<div class="loading">🔍 Looking up location...</div>';
    // Use the new API with HTTPS support
    fetch(`https://freegeoip.app/json/${encodeURIComponent(ip)}`)
        .then(res => res.json())
        .then(data => {
            if (data && data.ip) {
                displayLocation(data);
            } else {
                resultDiv.innerHTML = `<div class="error-msg">❌ ${data.message || 'Invalid IP or location not found.'}</div>`;
            }
        })
        .catch(err => {
            console.error('Error fetching location:', err);
            resultDiv.innerHTML = '<div class="error-msg">Network error. Please try again.</div>';
        });
});
</script>
</body>
